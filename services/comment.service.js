...
async function newComment(params, user, req, res) {
    const onComment = require('../sockets/feedANDcomments/feed').getCalls().onComment
    ...
    setTimeout(() => onComment(params.post_id), 800) // revisar pq se ejecuta antes de guardadoen la bd
}
...
async function searchComments(req) {
    ...
    if (typeof req.query.search != 'undefined') {
        ...
        comments = await db.Comment.findAll({
            where: {
                createdAt: { [Op.gt]: sevenDaysAgo, [Op.lt]: today }, [Op.or]: [
                    { title: { [Op.like]: '%' + req.query.search + '%' } },
                    { content: { [Op.like]: '%' + req.query.search + '%' } }
                ]
            }, include: ['profile']
